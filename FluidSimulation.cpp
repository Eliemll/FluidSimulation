...
void FluidSimulation::update(FluidSimulation fluidSimulation,float SmoothingRadius, float dt, float Damping)
{
    // setConfinementRect(rect);
    for (auto &particle : m_particles)
    {
        // Utiliser la position de la particule comme vecteur 2D
        sf::Vector2f samplePoint = particle.getPosition();

        particle.updatePosition(fluidSimulation, samplePoint, SmoothingRadius, dt, Damping);
    }
}
